(function () {
  ...
  class GRenderer {
    constructor(canvas, disableimageSmoothing) {
      if (canvas) {
        ...
        var obj = this;
        this.mouseDetectionEnabled = true;
        this.canvas.onmousemove = function (e) {
          if (obj.mouseDetectionEnabled) {
          ...
          if (obj.mouseDetectionEnabled) {
            ...
            for (var spr of obj.clickDetectorSprites) {
              ...
              if (sprTouching) {
                ...
                spr.events.click.forEach((f) => {
                  f(obj.mousePos[0], obj.mousePos[1]);
                });
              }
            }
            if (!spriteDetectorClicked) {
              obj.events.mousedown.forEach((f) => {
                f(obj.mousePos[0], obj.mousePos[1]);
              });
            }
          }
        ...
        throw Error(
          "No canvas was specified, please use validly: new GRenderer(canvasElement)"
        );
      }
    }
	checkIsOffscreen(x,y,w,h,sw,sh) {
		if (!this.doOffscreenCheck) {
			return false;
		}
		//return (
        //   (x + w) < 0 
    ...
    checkCollisions(x1, y1, w1, h1, x2, y2, w2, h2) {
      //square collision
    }
    checkSpriteCollision(spr1, spr2) {
      ...
      var a = {
        x: this.xToLeft(spr1.x, spr1.width * spr1.scale),
        y: this.yToTop(spr1.y, spr1.height * spr1.scale),
        width: spr1.width * spr1.scale,
        height: spr1.scale * spr1.height,
      };
      var b = {
        x: this.xToLeft(spr2.x, spr2.width * spr2.scale),
        y: this.yToTop(spr2.y, spr2.height * spr2.scale),
        width: spr2.width * spr2.scale,
        height: spr2.scale * spr2.height,
      };
      //console.log(a,b);
      //debug
    ...
    drawSprites(sprList) {
      ...
      for (var sprite of sprList) {
        ...
        try {
          if (sprite.positiontype.toLowerCase() == "scratch") {
            this.ctx.translate(
              (sprite.x + this.offsetDrawX + this.gameScreenWidth / 2) *
                this.scaleX,
              (sprite.y + this.offsetDrawY + this.gameScreenHeight / 2) *
                this.scaleY
            );
          }
          if (sprite.positiontype.toLowerCase() == "top-left") {
            this.ctx.translate(
              (sprite.x + this.offsetDrawX) * this.scaleX,
              (sprite.y + this.offsetDrawY) * this.scaleY
            );
          }
          var offsetx = 0;
          var offsety = 0;
          ...
          if (!offscreen) {
            var drawinfo = [
              Math.round(offsetx * this.scaleX * sprite.scale),
              Math.round(offsety * this.scaleY * sprite.scale),
              Math.round(sprite.width * this.scaleX * sprite.scale),
              Math.round(sprite.height * this.scaleY * sprite.scale),
            ];
            ...
            if (sprite.visible) {
              if (sprite.type == "norm") {
                ...
                if (sprite.imageLocation) {
                  ...
                  if (sprite.imageLocation.offset) {
                    drawX += sprite.imageLocation.offset[0] * this.scaleX;
                    drawY += sprite.imageLocation.offset[1] * this.scaleY;
                  }

                  var imageLocation = sprite.imageLocation;
				  if (
					!this.checkIsOffscreen(
					drawX,
					drawY,
					drawWidth,
					drawHeight,
					this.gameScreenWidth * this.scaleX,
					this.gameScreenHeight * this.scaleY
					)){
						this.ctx.drawImage(
						sprite.image,
						imageLocation.x,
              ...
              if (sprite.type == "text") {
                try {
                  if (sprite.bold) {
                    this.ctx.font =
                      "bold " + sprite.size * this.scaleY + "px " + sprite.font;
                  } else {
                    this.ctx.font =
                      sprite.size * this.scaleY + "px " + sprite.font;
                  }
                  this.ctx.fillStyle = sprite.color;
				  var dx = drawX;
                  if (sprite.center) {
                    sprite.width = this.ctx.measureText(sprite.text).width;
					dx = drawX / this.scaleX / sprite.scale;
                  }
                  this.ctx.fillText(sprite.text, dx, drawY);
                } catch (e) {
                  window.alert(e);
                }
              }
      ...
      if (this.blackScreen) {
        this.ctx.fillStyle = "black";
		this.ctx.globalAlpha = this.blackScreenAlpha;
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
      }
    }
    createImage(src, name) {
      return new Promise((resolve, reject) => {
        ...
        image.onerror = function () {
          ...
          reject(`[GRenderer]: Failed To Load Image ${src.slice(0,20)}`);
        };
        image.src = src;
      });
    }
  ...
  window.GRender.Render = GRenderer;
  class Sprite {
  ...
  window.GRender.SquareSprite = SquareSprite;
  class TextSprite {
    constructor(x, y, image, width, height) {
		this.filter = "";
      this.visible = true;
  ...
  window.GRender.TextSprite = TextSprite;
})();
