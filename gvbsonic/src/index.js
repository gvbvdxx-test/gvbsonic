        ...
        if (!spr.inDebug) {
            if (!spr.htimeout && spr.hurt) {
                if (spr.rings > 0) {
                    ...
                    //This is changed because of the "Sonic 4 feel" it has, fixed to be less like Sonic 4.
                    if (spr.speed > 0.5) {
                        //The player is not moving, so just use the opposite direction of where the player is.
                        if (spr.flipX) {
                            spr.speed = 7;
                        } else {
                            spr.speed = -7;
                        ...
                        //Move the player in the opposite direction of where they are going.
                        if (spr.speed > 0) {
                            spr.speed = -7;
                        } else {
                            spr.speed = 7;
            ...
            if (!spr.inDebug) {
                ...
                if (true) {
                    if (!spr.dead) {
                            ...
                            if (!(spr.left || spr.right)) {
                                ...
                                if (spr.onfloor) {
                                    if (Math.round(spr.engineAngle) == 90) {
                                        ...
                                        spr.speed += Math.sin(degrees_to_radians(spr.engineAngle - 90)) * 0.125;
                                    }
                                } else {
                                    if (Math.abs(spr.speed) < 1) {
                                        spr.speed = 0;
                        ...
                        if (spr.onfloor) {
                            if (Math.abs(spr.speed) < 1) {
                                //I dont think other engines make it 90 when no inputs are pressed.
...
window.startEngine = async function startEngine(
    ...
    sonic.onhurt = async function (r) {
        //Should fix the wierd death bug with rings.
        if (!sonic.dead) {
