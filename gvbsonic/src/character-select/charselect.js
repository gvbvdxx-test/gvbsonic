window.doCharacterSelect = async function (backtomenu,next) {
	...
	function replayAnimation () {
		if (charInfo[selectedID]) {
			...
			if (characterView.animationData) {
				if (characterView.animationData["stand"]) {
					var anim = characterView.animationData["stand"];
					characterView.imageLocation = getSpriteByName(characterView.spritesheetData, anim.frames[0]);
					characterView.width = characterView.imageLocation.width;
					characterView.height = characterView.imageLocation.height;
				}
			}
		}
	}
